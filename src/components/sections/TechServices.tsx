...
export default function TechServices() {
  return (
    <section className="relative min-h-screen py-24 px-4 overflow-hidden z-20 flex flex-col md:flex-row items-center justify-center bg-surface">
      ...
      <div className="flex-1 max-w-xl md:pr-12 md:ml-12 text-center md:text-left mb-16 md:mb-0 z-10">
        ...
        <p className="text-[var(--muted)] mb-8 max-w-md mx-auto md:mx-0">
          From cutting-edge corporate portals to complex pharma-tech integrations, Milarch Tech delivers architecture that scales.
        </p>
        <button className="px-8 py-4 bg-electric text-white font-semibold rounded-full hover:bg-electric/90 hover:shadow-[0_0_20px_rgba(43,178,194,0.4)] transition-all">
          View All Services
        </button>
      </div>
      ...
      <div className="flex-1 w-full relative min-h-[400px] md:min-h-[600px] flex items-center justify-center">
        <div className="relative w-[300px] h-[300px] md:w-[400px] md:h-[400px]">
          <svg
            className="absolute inset-0 w-full h-full text-electric/30"
            viewBox="0 0 100 100"
          >
            <motion.path
          ...
          {services.map((service, i) => {
            ...
            return (
              ...
              >
                <div className="w-24 h-24 bg-[var(--card)] border border-electric/40 flex items-center justify-center relative cursor-pointer group-hover:bg-electric/10 transition-colors shadow-sm [clip-path:polygon(50%_0%,100%_25%,100%_75%,50%_100%,0%_75%,0%_25%)]">
                  <span className="text-foreground font-bold text-center text-xs px-2">{service.title}</span>
                </div>

                {/* Tooltip */}
                <div className="absolute top-full left-1/2 -translate-x-1/2 mt-4 bg-[var(--card)] border border-[var(--border)] shadow-md p-3 rounded-lg w-48 opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity text-center z-20">
          ...
          {/* Central node */}
          <motion.div
            className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-32 h-32 bg-electric shadow-[0_0_40px_rgba(43,178,194,0.4)] flex items-center justify-center [clip-path:polygon(50%_0%,100%_25%,100%_75%,50%_100%,0%_75%,0%_25%)] z-10"
            initial={{ scale: 0 }}
            whileInView={{ scale: 1 }}
            viewport={{ once: true }}
